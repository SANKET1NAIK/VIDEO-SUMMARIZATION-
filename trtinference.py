...
class HorseGaitMonitor:
    ...
    def detect_state(self, keypoints):
        """Detect the horse's state based on keypoints, adjusted for top-diagonal view."""
        ...
        for person_id, kp_array in keypoints.items():
            ...
            # Adjusted for top-diagonal view: Pawing detection
            # Relaxed angle range since legs may appear less bent from above
            if 80 <= left_front_angle <= 120 or 80 <= right_front_angle <= 120:
                return "pawing"

            # Adjusted for top-diagonal view: Sitting detection
            # Lowered threshold since legs may not appear as sharply bent
            if all(
                angle < 70  # Reduced from 90 to account for perspective
                for angle in [
                    left_front_angle,
                    right_front_angle,
                    left_back_angle,
            ...
            # Check for walking/standing with adjusted movement threshold
            ...
            if self.prev_positions is not None:
                ...
                # Increased threshold to reduce false positives for walking
                avg_movement = np.mean(self.movement_buffer)
                movement_detected = avg_movement > 8.0  # Raised from 5.0
    ...
    def draw_state_annotation(self, frame, state):
        """Draw state annotation on the frame."""
...
def main():
    ...
    try:
        ...
        monitor.process_video("E:/vitpose/walk.mp4")
